...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function render() {
            ...
            var config = { view: option.view, weekstartday: option.weekstartday, theme: option.theme, harilibur : option.harilibur, ketlibur : option.ketlibur, absensihari : option.absensihari, ketabsensi:option.ketabsensi};
        ...
        function BuildDaysAndWeekView(startday, l, events, config) {
            ...
            html.push("<div id=\"dvtec\"  class=\"scolltimeeventxx\"><table style=\"table-layout: fixed;", jQuery.browser.msie ? "" : "width:100%", "\" cellspacing=\"0\" cellpadding=\"0\"><tbody><tr><td>");
        ...
        function BuildMonthView(showday, events, config) {
            ...
            BuilderMonthBody(html, showday, config.weekstartday, events, bH, config.harilibur, config.ketlibur, config.absensihari, config.ketabsensi);
            html.push("</div>");
            gridcontainer.html(html.join(""));
            html = null;
        ...
        function BuilderMonthBody(htb, showday, startday, events, bodyHeight, harilibur, ketlibur, absensihari, ketabsensi) {
			...
			var libur = harilibur.split(',');
			var ketlbr = ketlibur.split(',');
			var absensi = absensihari.split(',');
			var ketabsen = ketabsensi.split(',');
                ...
                for (var i = 0; i < 7; i++) {
                    var day = C[j * 7 + i];
                     htb.push("<td abbr='", dateFormat.call(day, i18n.xgcalendar.dateformat.fulldayvalue), "' ch='qkadd' axis='00:00' title=''");
 					 var warnaLibur =0;
					 var warnabsen =0;
 					 for(var k=0; k <= libur.length-1; k++)
					 {
 						 if(libur[k] == dateFormat.call(day, "yyyyMMdd") && warnabsen ==0) {
							 warnaLibur = 1; 
							 htb.push(" class=\" st-bg st-bg-libur\">" + ketlbr[k]);    
						 } 
 					 }	
					 
					 for(var v=0; v <= absensi.length-1; v++)
					 {
 						 if(absensi[v] == dateFormat.call(day, "yyyyMMdd") && warnaLibur ==0) {
							 warnabsen = 1; 
							 htb.push(" class=\" st-bg st-bg-absensi\">" + ketabsen[v]);    
						 } 
 					 }	 
					
                    if (dateFormat.call(day, "yyyyMMdd") == dateFormat.call(new Date(), "yyyyMMdd")  && warnaLibur==0 && warnabsen ==0) {
 						htb.push(" class=\" st-bg-today\">");
                     }
                    else {
 					    
						if(i==6 && warnaLibur==0 && warnabsen ==0)
						{ 
						    htb.push(" class=\"st-bg\" style='background:#CCC'>"); 
 						} 
						
						if(i==5 && warnaLibur==0 && warnabsen ==0)
						{
 						    htb.push(" class=\"st-bg\" style='background:#CCC'>"); 
 						}
						
						if(i!=6 && i!=5 && warnaLibur==0 && warnabsen ==0)
						{
        ...
        function populate() {
            ...
            if (option.url && option.url != "") {
                ...
                { name: "viewtype", value: option.view },
				 ...
				 { name: 'ci_csrf_token', value: $('#csrftoken').val() }
                ];
                if (option.extParam) {
