...
<p><br/><a href="<?php echo site_url(); ?>">&larr; <?php echo lang('us_back_to') . $this->settings_lib->item('site.title'); ?></a></p>
        ...
        <h2 class='login_title text-center'><img src="<?php echo base_url();?>upload/Lambang_Polri.png" width="100"> <br>Login</h2>
        <hr>
            ...
            <?php echo form_open(LOGIN_URL, array('autocomplete' => 'off','class'=>"form-signin")); ?> 
                <span id="reauth-email" class="reauth-email <?php echo iif( form_error('login') , 'error') ;?>"></span>
                <p class="input_title">Username</p>
                <input type="text" name="login" id="inputEmail" class="login_box" value="<?php echo set_value('login'); ?>" tabindex="1" placeholder="<?php echo $this->settings_lib->item('auth.login_type') == 'both' ? lang('bf_username') .'/'. lang('bf_email') : ucwords($this->settings_lib->item('auth.login_type')) ?>" />
                
				<span id="reauth-email" class="reauth-email <?php echo iif( form_error('password') , 'error') ;?>"></span>
				<p class="input_title">Password</p>
                <input type="password" name="password"  id="inputPassword" class="login_box" placeholder="<?php echo lang('bf_password'); ?>" required>
