...
class Contexts
{
    ...
    protected static $templateContextMenu = "<li class='{parent_class}'><a href='{url}' id='{id}' class='{current_class}' title='{title}' {extra}><span class='nav-label'>{text}</span> </a>{content}</li>\n";
    ...
    protected static $templateContextEnd             = "<span class='fa arrow'></span>";
    protected static $templateContextImage           = "<i class='fa fa-files-o'></i>{title}";//"<img src='{image}' alt='{title}' />";
    ...
    protected static $outer_class = 'nav nav-list';
    ...
    protected static $child_class = 'nav nav-second-level collapse';
        ...
        foreach (Modules::list_modules() as $module) {
            if (Modules::controller_exists($context, $module)) {
                ...
                self::$actions[$module] = array(
                    'id_display'   => isset($mod_config['id']) ? $mod_config['id'] : $module,
    ...
    private static function sortActions()
    {
        ...
        foreach (self::$actions as $key => $action) {   
			$weights[$key]       = $action['id_display'];
            $display_names[$key] = $action['display_name'];
        }

        array_multisort($weights, SORT_ASC, $display_names, SORT_DESC, self::$actions);
    }
